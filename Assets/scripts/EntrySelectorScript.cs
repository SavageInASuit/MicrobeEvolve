...
public class EntrySelectorScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse0))
        {
            ...
            DistanceEntryScript entry = null;

            //For every result returned, output the name of the GameObject on the Canvas hit by the Ray
            foreach (RaycastResult result in results)
            {
                if(result.gameObject.tag == "DistanceEntry")
                {
                    entry = result.gameObject.GetComponent<DistanceEntryScript>();
                    break;
                }
            }
